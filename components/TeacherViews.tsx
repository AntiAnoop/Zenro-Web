...
import { Users, BookOpen, Clock, Plus, Video, BarChart2, Calendar, FileText, CheckCircle, AlertTriangle, X, Loader2, FileCheck, Layers, Radio, Trash2, Edit3, Save, ChevronLeft, ChevronRight, Repeat } from 'lucide-react';
        ...
        <div className="space-y-8 animate-fade-in">
            <div className="flex justify-between items-center bg-white p-8 rounded-3xl border border-gray-100 shadow-sm">
                <div>
                    ...
                    <p className="text-gray-400 font-medium">Verified database aggregation for your assigned batches.</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="bg-white rounded-3xl border border-gray-100 p-8 shadow-sm">
                    <h3 className="text-xl font-black text-zenro-blue mb-8 flex items-center gap-3"><FileCheck className="w-6 h-6 text-green-500" /> Proctoring Stream</h3>
                    <div className="p-10 text-center border-2 border-dashed border-gray-50 rounded-3xl"><AlertTriangle className="w-10 h-10 text-gray-100 mx-auto mb-4" /><p className="text-gray-300 font-bold text-xs uppercase tracking-widest">Awaiting active exam submissions...</p></div>
                </div>
            </div>
        </div>
    );
...
export const TeacherSchedulePage = () => {
    ...
    useEffect(() => { fetchSchedule(); }, [currentMonth]);
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            <div className="flex justify-between items-center bg-white p-8 rounded-3xl border border-gray-100 shadow-sm">
                ...
                <button onClick={() => setIsModalOpen(true)} className="bg-zenro-blue text-white px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl">Schedule Lecture</button>
            </div>
            <div className="bg-white rounded-[40px] border border-gray-100 shadow-sm overflow-hidden grid grid-cols-7">
                ...
                {Array.from({ length: 35 }).map((_, i) => {
                    ...
                    return (
                        <div key={i} className={`min-h-[120px] p-4 border-r border-b border-gray-50 ${isValid ? 'hover:bg-blue-50/20' : 'bg-gray-50/10'}`}>
                            {isValid && <span className="font-black text-slate-300 text-sm">{day}</span>}
                            <div className="mt-2 space-y-1">
                                {dayEvents.map(ev => <div key={ev.id} className="bg-zenro-blue text-white p-1 rounded text-[9px] truncate font-bold">{ev.title}</div>)}
                            </div>
                        </div>
                    );
                })}
...
const ScheduleModal = ({ onClose, onRefresh, teacherId }: any) => {
    ...
    const handleSave = async () => {
        if (!title || !batch) return alert("Fields required");
        setLoading(true);
    ...
    return (
        <div className="fixed inset-0 z-50 bg-slate-900/80 backdrop-blur-xl flex items-center justify-center p-4">
            <div className="bg-white w-full max-w-lg rounded-[40px] p-10 space-y-8">
                <h2 className="text-3xl font-black text-zenro-blue tracking-tighter">Sync Lecture</h2>
                <div><label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Topic</label><input className="w-full bg-gray-50 border border-gray-100 p-4 rounded-2xl" value={title} onChange={e => setTitle(e.target.value)} /></div>
                <div><label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Batch</label>
                    <select className="w-full bg-gray-50 border border-gray-100 p-4 rounded-2xl" value={batch} onChange={e => setBatch(e.target.value)}>
                ...
                <div className="flex justify-end gap-4"><button onClick={onClose} className="px-6 font-bold">Cancel</button><button onClick={handleSave} disabled={loading} className="bg-zenro-red text-white px-10 py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl">Sync Class</button></div>
            </div>
        </div>
    );
};
...
export const TeacherAssignmentsPage = () => {
    ...
    return (
        <div className="space-y-8 animate-fade-in">
            <div className="flex justify-between items-center bg-white p-8 rounded-3xl border border-gray-100 shadow-sm"><h1 className="text-3xl font-heading font-black text-zenro-blue tracking-tighter">Assignments</h1><button className="bg-zenro-red text-white px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-widest"><Plus className="w-4 h-4" /> Create Task</button></div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {loading ? <Loader2 className="animate-spin mx-auto"/> : assignments.map(a => (
                    <div key={a.id} className="bg-white p-8 rounded-[40px] border border-gray-100 shadow-sm">
                        <FileText className="w-12 h-12 text-blue-100 mb-6" />
                        <h3 className="text-xl font-bold mb-2">{a.title}</h3>
                        <p className="text-xs text-gray-400 font-black uppercase mb-4">Batch: {a.assignment_batches?.map((b:any) => b.batch_name).join(', ')}</p>
                        <button className="w-full bg-gray-50 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-zenro-blue hover:text-white transition-all">Review Submissions</button>
                    </div>
                ))}
            </div>
        </div>
...
export const TeacherTestsPage = () => {
    ...
    return (
        <div className="space-y-8 animate-fade-in">
            <div className="flex justify-between items-center bg-white p-8 rounded-3xl border border-gray-100 shadow-sm"><h1 className="text-3xl font-heading font-black text-zenro-blue tracking-tighter">Test Engine</h1><button className="bg-zenro-red text-white px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-widest"><Plus className="w-4 h-4" /> New Exam</button></div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {loading ? <Loader2 className="animate-spin mx-auto"/> : tests.map(t => (
                    <div key={t.id} className="bg-white p-8 rounded-[40px] border border-gray-100 shadow-sm group hover:shadow-xl transition-all">
                        ...
                        <button className="w-full bg-slate-900 text-white py-3 rounded-xl text-[10px] font-black uppercase tracking-widest">Manage Questions</button>
                    </div>
                ))}
            </div>
        </div>
...
export const TeacherCoursesPage = () => {
    ...
    return (
        <div className="space-y-8 animate-fade-in">
            <div className="flex justify-between items-center bg-white p-8 rounded-3xl border border-gray-100 shadow-sm"><h1 className="text-3xl font-heading font-black text-zenro-blue tracking-tighter">Curriculum Repository</h1><button className="bg-zenro-blue text-white px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-widest"><Plus className="w-4 h-4" /> New Course</button></div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {loading ? <Loader2 className="animate-spin mx-auto"/> : courses.map(c => (
                    <div key={c.id} className="bg-white rounded-[40px] border border-gray-100 overflow-hidden shadow-sm flex group hover:shadow-2xl transition-all">
                        <img src={c.thumbnail} className="w-48 h-full object-cover grayscale group-hover:grayscale-0 transition-all" alt="" />
                        <div className="p-8 flex-1">
                            ...
                            <div className="flex justify-between items-center pt-4 border-t border-gray-50"><span className="text-[10px] font-black uppercase text-gray-300">Batch: {c.course_batches?.[0]?.batch_name}</span><button className="text-zenro-blue font-black text-[10px] uppercase">Edit Content</button></div>
                        </div>
                    </div>
                ))}
            </div>
...
export const TeacherReportsPage = () => <div className="p-10 text-center text-gray-300 font-bold uppercase tracking-widest italic animate-pulse">Synchronizing Analytics Hub...</div>;
export const LiveClassConsole = () => <div className="h-[80vh] flex flex-col items-center justify-center bg-slate-900 rounded-[50px] shadow-2xl relative overflow-hidden"><div className="absolute inset-0 bg-gradient-to-br from-zenro-blue/40 to-transparent"></div><div className="relative z-10 text-center text-white"><Video className="w-16 h-16 text-zenro-red mx-auto mb-10 animate-pulse" /><h2 className="text-4xl font-heading font-black tracking-tighter mb-4">Encrypted Broadcast Feed</h2><button className="bg-white text-zenro-blue px-12 py-5 rounded-3xl font-black text-xs uppercase tracking-[0.2em] shadow-2xl hover:scale-105 active:scale-95 transition-all">Go Live Now</button></div></div>;
export const CourseContentManager = () => <div className="p-10">Editor active...</div>;
